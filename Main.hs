...
instance Show (a n) => Show (FreeAb a n) where
  show (FreeAb a) = show a

-- instance Ord (a NZ) => Ord (FreeAb a NZ) where compare = undefined
-- instance Ord (a (NS n)) => Ord (FreeAb a (NS n)) where compare = undefined
...
simplifyFreeAb (FreeAb coeffs) = case M.toList $ M.fromListWith (+) coeffs of
  coeffs' -> FreeAb $ [(a, c) | (a, c) <- coeffs', c /= 0]
...
fromBoundary :: [DiscreteManifold b n] -> DiscreteManifold b  (NS n)
...
-- pullbackForm fwd (Differential f) = Differential undefined
...
data RandomVariable a 
  = Return a
  | Sample01 (Bool -> RandomVariable a)
  deriving Functor

instance Monad RandomVariable where
  Return a >>= a2mb = a2mb a
  Sample01 maproducer >>= a2mb = Sample01 $ maproducer >=> a2mb
